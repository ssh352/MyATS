...
namespace feed
{
	namespace zd
	{
		zd_source::zd_source(const string & sourceName, const string & hostname, const string & service, const string & brokerId, const string & user, const string & password, const string & dbName, const string& authServerIp, const string& authServerPort, const string& authCode, string pub, string url, string req_url)
			: feed_source("ZD", sourceName, hostname, service, brokerId, user, password, dbName, pub, url,req_url)
		{
			m_pConnection = new zd_connection(this);
			//
			m_auth_server_ip = authServerIp;
			m_auth_port      = authServerPort;
			m_auth_code      = authCode;
			//
		}

		void zd_source::init_source()
		{
			//
			feed_source::init_source();
			//

			get_queue()->setHandler(std::bind(&zd_source::process_msg, this, std::placeholders::_1));

			//std::thread t(std::bind(&zl_source::set_kernel_timer_thread, this));
			//m_thread.swap(t);
...
#ifdef Linux
		void  zd_source::init_epoll_eventfd()
		{
			efd = eventfd(0, EFD_NONBLOCK);
			if (-1 == efd)
			{
				cout << "zd_source efd create fail" << endl;
				exit(1);
			}

			add_fd_fun_to_io_service(io_service_type::feed, efd, std::bind(&zd_source::process, this));
			m_queue.set_fd(efd);
		}
#endif
		void zd_source::start_receiver()
		{
			CTShZdDepthMarketDataField msg;
			int rc = -1;
			//while (true)
			{
		...
		//void zl_source::release_source()
		//{
		//	m_pConnection->cleanup();					
		//}
		void zd_source::process_msg(CTShZdDepthMarketDataField* pMsg)
		{
			std::string sFeedCode = std::string(pMsg->InstrumentID);
			//std::string instrName = get_code_by_feedcode(sFeedCode);
			//if (instrName.empty())
			//{
			//	loggerv2::error("zl_source::process_msg cannot find Code for FeedCode %s.", sFeedCode.c_str());
			//	return;
			//}
			feed_item* afeed_item = get_feed_item(sFeedCode);
			if (afeed_item == NULL)
			{
				//loggerv2::error("zl_source::process_msg instrument %s not found", pMsg->InstrumentID);
				return;
			}
		...
		void zd_source::process()
		{
			get_queue()->Pops_Handle();
		}


		void zd_source::update_item(CTShZdDepthMarketDataField* pMsg, feed_item * feed_item)
		{
			...
			double closePrice = (!(pMsg->ClosePrice == NO_PRICE || pMsg->ClosePrice == 0)) ? pMsg->ClosePrice : (pMsg->PreSettlementPrice != NO_PRICE ? pMsg->PreSettlementPrice : 0);
			...
			int dailyVolume = pMsg->TotalVolume;
