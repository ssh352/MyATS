...
namespace ht_file
{
	void ht_file_order_aux::anchor(ht_file_connection* pConnection, result_orde* pOrder, std::string &path, std::list<order *> &mlist)
	{
		auto fn = pOrder->file_name;
		boost::filesystem::path p;
		p.clear();
		p.append(path);
		p.append(fn);

		auto rtn_name = p.string();
		...
		while (stream.good())
		{
			...
			if (vec[2] == "1")
				...
				exchange_type = "SZE";
			auto stock_code = vec[3];
			auto way = vec[4];
			double price = boost::lexical_cast<double>(vec[6]);
			int qty = boost::lexical_cast<int>(vec[7]);
			
			std::string sInstrCode = stock_code + "." + exchange_type + "@" + pConnection->getName();

			tradeitem* instr = tradeitem_gh::get_instance().container().get_by_second_key(sInstrCode.c_str());
			if (instr == NULL)
			{
				loggerv2::error("lts_order::anchor - tradeitem [%-*.*s] not found", sizeof(stock_code), sizeof(stock_code), stock_code);
				return;
			}
			...
			pConnection->get_user_info(vec[8].c_str(), nAccount, nUserOrdId, nInternalRe, nPortfolio, nTradingType);
			int	id = nInternalRe;
			if (id < 1)
			{
				id = FAKE_ID_MIN + boost::lexical_cast<int>(vec[0]);;
			}
			// ok
			o = pConnection->get_order_from_pool();
			o->set_id(id);
			...
			AtsType::OrderOpenClose::type oc = AtsType::OrderOpenClose::Open;
			o->set_open_close(oc);
