...
namespace lts
{
	lts_reqapi::lts_reqapi(lts_connection* pConnection)
	{
...
//>>>>>>> nas/x1
	...
	lts_reqapi::~lts_reqapi()
	{

	}


	void lts_reqapi::Process()
	{
	
	}
	
	void lts_reqapi::request_login(CSecurityFtdcAuthRandCodeField *pAuthRandCode)
	{
		/*if (m_pConnection->m_blts_wrapper)
			return;*/
		loggerv2::info("lts_reqapi::request_login");
		...
		loggerv2::info("lts_reqapi::request_login broker %s, user %s, ranCode %s, password %s", m_pConnection->m_sBrokerId.c_str(), m_pConnection->m_sUsername.c_str(), pAuthRandCode->RandCode, m_pConnection->m_sPasswordQry.c_str());
	...
	void lts_reqapi::request_auth()
	{
		/*if (m_pConnection->m_blts_wrapper)
		{
			return;
		}*/
		loggerv2::info("lts_reqapi::request_auth.");
	...
	void lts_reqapi::OnRspQryInstrument(CSecurityFtdcInstrumentField *pInstrument, CSecurityFtdcRspInfoField *pRspInfo, int nRequestID, bool bIsLast)
	{
		if (pInstrument == nullptr)
			return;	
		if (pInstrument->ProductClass == '2') //Options
			 {
			loggerv2::info("lts_reqapi::OnRspQryInstrument"
				 " CreateDate[%s]"
				 "DeliveryYear[%d]"
				 "DeliveryMonth[%d]"
				 "EndDelivDate[%s]"
			...
			else //exists
			{
				...
				std::string sConnectionCodes = std::string(pInstrument->InstrumentID) + "." +std::string(pInstrument->MarketID) + "@" + m_pConnection->get_type();
		...
		if (bIsLast)
		{
			...
			this->m_pConnection->set_is_last(true);			
		}

	}


	std::string lts_reqapi::getMaturity(std::string sMat)
	{
		std::string newMat;
	...
	void lts_reqapi::getOptionsFeatures(const char* Code, std::string& underlying, std::string& CallPut, std::string& instrumentCalss)
	{
		char sUnder[6 + 1];
		char sCallPut[1 + 1];
	...
	void lts_reqapi::OnRspQryInvestorPosition(CSecurityFtdcInvestorPositionField *pInvestorPosition, CSecurityFtdcRspInfoField *pRspInfo, int nRequestID, bool bIsLast)
	{
	if (bIsLast == true)
		{
			...
			{				
				...
				int ret = m_pConnection->request_instruments();
				loggerv2::error("lts_reqapi::OnRspQryInvestorPosition: request_instruments,ret:%d\n",ret);				
			}
			else
			{
				sleep_by_milliseconds(2000);
		...
		/*if (pInvestorPosition != NULL)
			loggerv2::info("lts_reqapi::OnRspQryInvestorPosition InstrumentID[%s]"
			"ExchangeID[%s]"
			"PosiDirection[%c]"
			"YdPosition[%d]"
			"Position[%d] "
...
*/
		if (pInvestorPosition->InstrumentID)
		{
			...
			std::string sInstrCode = m_pConnection->compute_second_key(std::string(pInvestorPosition->InstrumentID), std::string(pInvestorPosition->ExchangeID));
			...
			if (i)
			{
				...
				loggerv2::warn("lts_reqapi::OnRspQryInvestorPosition cannot find instrument %s", pInvestorPosition->InstrumentID);
		}



	}
	void lts_reqapi::OnRspQryTradingAccount(CSecurityFtdcTradingAccountField *pTradingAccount, CSecurityFtdcRspInfoField *pRspInfo, int nRequestID, bool bIsLast)
	{
		...
		if (pTradingAccount->CurrMargin + pTradingAccount->Available != 0)
		{
			ratio = pTradingAccount->CurrMargin / (pTradingAccount->CurrMargin + pTradingAccount->Available);
		}

		loggerv2::info("lts_reqapi::OnRspQryTradingAccount,"
		...
		//
		if (SECURITY_FTDC_AcT_Derive == pTradingAccount->AccountType)
		{
	...
	int lts_reqapi::request_instruments()
	{
		...
		-3，表示每秒发送请求数超过许可数。
		*/
		int ret = m_pQueryApi->ReqQryInstrument(&request, ++m_nRequestId);
		printf_ex("lts_reqapi::request_instruments,ret:%d\n",ret);
		loggerv2::info("lts_reqapi_connection::request_instruments,ret:%d\n",ret);
		return ret;
	}

	bool lts_reqapi::ReqQryTradingAccount(CSecurityFtdcQryTradingAccountField *pQryTradingAccount)
	{
		int ret = m_pQueryApi->ReqQryTradingAccount(pQryTradingAccount, ++m_nRequestId);
		loggerv2::info("lts_reqapi::ReqQryTradingAccount brokerid %s , requestId %d,ret:%d", pQryTradingAccount->BrokerID, m_nRequestId,ret);
		if (ret != 0)
		{
			return false;
		}
	...
	bool lts_reqapi::ReqQryInvestorPosition(CSecurityFtdcQryInvestorPositionField *pQryInvestorPosition)
	{
	...
	void lts_reqapi::OnFrontConnected()
	{
		/*if (m_pConnection->m_blts_wrapper)
	...
	void lts_reqapi::OnRspFetchAuthRandCode(CSecurityFtdcAuthRandCodeField *pAuthRandCode, CSecurityFtdcRspInfoField *pRspInfo, int nRequestID, bool bIsLast)
	{
		/*if (m_pConnection->m_blts_wrapper)
			CSecurityFtdcQuerySpiBase::OnRspFetchAuthRandCode(pAuthRandCode, pRspInfo, nRequestID, bIsLast);*/
		loggerv2::info("lts_reqapi::OnRspFetchAuthRandCode - sending login request");
		request_login(pAuthRandCode);
	}
	
	void lts_reqapi::OnRspUserLogin(CSecurityFtdcRspUserLoginField *pRspUserLogin, CSecurityFtdcRspInfoField *pRspInfo, int nRequestID, bool bIsLast)
	{

		loggerv2::info("lts_reqapi::OnRspUserLogin - (%d, %s)", pRspInfo->ErrorID, pRspInfo->ErrorMsg);
		if (pRspInfo->ErrorID == 0)
		{
			set_status(true);
			loggerv2::info("lts_reqapi status set to true");
			...
			if (m_pConnection->m_bRequestPosition == false && m_pConnection->getRequestInstruments() == true)
			{
				int ret = m_pConnection->request_instruments();
				loggerv2::error("lts_reqapi::OnRspUserLogin:request_instruments,ret:%d\n",ret);
			}
		}
		else
		{
	...
	void lts_reqapi::OnRspUserLogout(CSecurityFtdcUserLogoutField *pUserLogout, CSecurityFtdcRspInfoField *pRspInfo, int nRequestID, bool bIsLast) 
	{
		loggerv2::info("calling lts_reqapi::OnRspUserLogout");
		m_bUserReqDiscon = true;
		if (pRspInfo->ErrorID == 0)
		{
			set_status(false);
			m_pConnection->on_status_changed(AtsType::ConnectionStatus::Disconnected, "lts_reqapi::OnRspUserLogout Receive Logout Msg Error Id 0");
		}

		else
			loggerv2::error("lts_reqapi::OnRspUserLogout logout failed ErrId[%d]", pRspInfo->ErrorID);
	...
	void lts_reqapi::OnFrontDisconnected(int nReason)
	{
		loggerv2::info("calling lts_reqapi::OnFrontDisconnected");
	...
	bool lts_reqapi::disconnect()
	{
		...
		loggerv2::info("lts_reqapi::disconnect - requesting logout.");
	...
	bool lts_reqapi::connect()
	{
		//loggerv2::info("lts_reqapi connecting...");
		...
		if (m_isAlive == false )//&& m_pConnection->m_blts_wrapper==false)
		{
			...
			loggerv2::info("lts_reqapi connecting to %s:%s", m_pConnection->m_sHostnameQry.c_str(), m_pConnection->m_sServiceQry.c_str());
		...
		{
			loggerv2::info("lts_reqapi::connect api is already alive, going to request auth");
			request_auth();
		}
		return true;
	}

	void lts_reqapi::init()
	{
		//m_pQueryApi = CSecurityFtdcQueryApi::CreateFtdcQueryApi("");
		//if (m_pConnection->m_blts_wrapper == false)
