...
namespace cffex
{
	...
	int cffex_connection::market_create_order_async(order* o, char* pszReason)
	{
		...
		if (o->get_way() != AtsType::OrderWay::Exercise)
		{
			...
			//add by wanyx 2017-12-25
			if (o->get_instrument()->get_instr_type() == AtsType::InstrType::ETF)//50ETF 只能开仓
				request->CombOffsetFlag[0] = THOST_FTDC_OF_Open;
...
void cffex_connection::OnRtnOrderAsync(CThostFtdcOrderField* pOrder)
{
	...
	get_user_info(pOrder->UserID, account, bidId, askId, portfolioId, ntradingType);//获取用户自定义字段
	...
	loggerv2::info("cffex_connection::OnRtnOrderAsync,orderID:%06d", orderId);

	int ret;
	order *o = get_order_from_map(orderId, ret);//找到orderID对应的原委托单
	switch (ret)
	{
	case 0:
		//o = reinterpret_cast<cffex_order*>(ord);
	...
	case 2:
		...
		o = cffex_order_aux::anchor(this, pOrder);//还原订单
		if (o == NULL)
		{
			loggerv2::error("cffex_connection::OnSOPEntrustOrderRtnAsyn cannot anchor order");
			return;
		...
		//}
	{
		...
		{
			//o->set_cancel_qty(pOrder->VolumeTotalOriginal - pOrder->VolumeTraded);//add by wanyx 2018-04-13 增加撤单数量
		...
		else if (o->get_last_action() == AtsType::OrderAction::Created || o->get_last_action() == AtsType::OrderAction::Cancelled)
		{
			if (o->get_status() == AtsType::OrderStatus::WaitMarket || o->get_status() == AtsType::OrderStatus::WaitServer)
			{
				...
				//update_instr_on_ack_from_market_cb(o);//mod by wanyx 2018-05-24 修改收到交易所回报时再设置订单为ACK状态
				...
				{
					//mod by wanyx 2018-05-24 修改收到交易所回报时再设置订单为ACK状态
					if (o->get_last_action() == AtsType::OrderAction::Created && !(o->custome_strings[0].empty()))
					{
						update_instr_on_ack_from_market_cb(o);
...

	{
			...
			// '5','2','4'
		{
			...
			{				
				//update_instr_on_ack_from_market_cb(o);
				if (pOrder->OrderStatus == THOST_FTDC_OST_Canceled)//add by wanyx 2018-04-13 确定为撤单状态撤单回调
				{
					update_instr_on_ack_from_market_cb(o);
					//o->set_cancel_qty(pOrder->VolumeTotalOriginal - pOrder->VolumeTraded);//add by wanyx 2018-04-13 增加撤单数量
					on_cancel_from_market_cb(o);
		...
		//mod by wanyx 2018-05-24 修改收到交易所回报时再设置订单为ACK状态
		else if (pOrder->OrderStatus == THOST_FTDC_OST_AllTraded || pOrder->OrderStatus == THOST_FTDC_OST_PartTradedQueueing || pOrder->OrderStatus == THOST_FTDC_OST_NoTradeQueueing)
		{
			if (o->get_last_action() == AtsType::OrderAction::Created && 
			   (o->get_status() == AtsType::OrderStatus::WaitMarket || o->get_status() == AtsType::OrderStatus::WaitServer))
			{
				loggerv2::info("cffex_connection::OnRtnOrderAsync order[%d] accepted status[%c]", o->get_id(),pOrder->OrderStatus);
				update_instr_on_ack_from_market_cb(o);
				on_ack_from_market_cb(o);
			}
		}
...
void cffex_connection::OnRtnTradeAsync(CThostFtdcTradeField* pTrade)
{
	// 0 - log
	//loggerv2::info();
	std::string strr = pTrade->TradeDate;
	if (strr.size() < 5)
		return;
...
#ifdef THOST_FTDC_PC_ETFOption
		if ((pInstrument->ProductClass != THOST_FTDC_PC_Options && pInstrument->ProductClass != THOST_FTDC_PC_Futures && pInstrument->ProductClass != THOST_FTDC_PC_SpotOption && pInstrument->ProductClass != THOST_FTDC_PC_ETFOption) || strlen(pInstrument->ExpireDate) < 1 /*|| strlen(pInstrument->ExchangeInstID) < 10*/)
#else
...
#ifdef THOST_FTDC_PC_ETFOption
		else if (pInstrument->ProductClass == THOST_FTDC_PC_ETFOption)//add by wanyx 2018-02-26
		{
			sInstr = string(pInstrument->InstrumentCode);
			sInstClass = "O_" + sUnderlying;
		}
#endif
