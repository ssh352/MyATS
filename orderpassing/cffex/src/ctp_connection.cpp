...
namespace cffex
{
	...
	void cffex_connection::init_connection()
	{
		...
		int trader_core = terra::ats::ats_config::get_instance()->get_trader_io_cpu_core();
		bool is_bind_feed_trader = terra::ats::ats_config::get_instance()->get_is_bind_feed_trader_core();

		if (trader_core > 0||(feed_core>0&&is_bind_feed_trader))
		{
			io_service_gh::get_instance().add_trader_update_handler(std::bind(&cffex_connection::process, this));
		}
		else
		{
			init_epoll_eventfd();
	...
	int cffex_connection::market_cancel_order_async(order* o, char* pszReason)
	{
		...
		if (o->get_way() != AtsType::OrderWay::Exercise)
		{
			...
			request->FrontID = o->custome_ints[1];
			request->SessionID = o->custome_ints[2];
			
			strcpy(request->OrderRef,o->custome_strings[1].data());			
			if (!compute_userId(o, request->UserID, sizeof(request->UserID)))
			{
				xs_cancel_pool.free_mem(request);
				return -1;
	...
	o->custome_ints[1]=pOrder->FrontID;
	o->custome_ints[2]=pOrder->SessionID;
	o->custome_strings[1]=pOrder->OrderRef;
