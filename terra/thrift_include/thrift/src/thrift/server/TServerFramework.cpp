...
void TServerFramework::newlyConnectedClient(const boost::shared_ptr<TConnectedClient>& pClient) {
  ...
  //printf("new client:%d is connected! and the total client num is :%d\n", (int)pClient.get(), clients_);
}
...
void TServerFramework::disposeConnectedClient(TConnectedClient* pClient) {
  ...
  //printf("the client:%d is disconnected! and the total client num is :%d\n", (int)pClient, clients_);
  delete pClient;
}
}
}
